...
defaultConfig =
  Config
    (\f _ _ _ -> f >> pure (constDyn ()))
    (\f _ _ -> f)
    (dyn_ . fmap elPandocRawNodeSafe)

-- | Convert Markdown to HTML
elPandoc :: forall t m a. (DomBuilder t m, Monoid a, MonadHold t m, MonadFix m, PostBuild t m) => Config t m a -> Dynamic t Pandoc -> m (Dynamic t a)
elPandoc cfg doc = do
  let fs = queryFootnotes <$> doc
  x <- flip runReaderT fs $ renderBlocks cfg (pandocBlocks <$> doc)
  f <- renderFootnotes (sansFootnotes . renderBlocks cfg . fmap (fmap blockToDSum)) fs
  pure (x <> join f)
...
elPandocInlines :: (PostBuild t m, MonadHold t m, MonadFix m, DomBuilder t m) => Dynamic t [Inline] -> m ()
elPandocInlines = void . sansFootnotes . renderInlines defaultConfig . fmap (fmap inlineToDSum)

-- | Render list of Pandoc Blocks
elPandocBlocks :: (PostBuild t m, DomBuilder t m, MonadHold t m, MonadFix m) => Dynamic t [Block] -> m ()
elPandocBlocks = void . sansFootnotes . renderBlocks defaultConfig . fmap (fmap blockToDSum)

mapAccum :: (DomBuilder t m, MonadHold t m, PostBuild t m, MonadFix m, Monoid a) => Dynamic t [v] -> (Dynamic t v -> m (Dynamic t a)) -> m (Dynamic t a)
mapAccum xs f = do
  r <- simpleList xs f
  pure (mconcat =<< r)

mapAccumIndex :: (DomBuilder t m, PostBuild t m, MonadFix m, MonadHold t m, Monoid a) => Dynamic t [v] -> (Int -> Dynamic t v -> m (Dynamic t a)) -> m (Dynamic t a)
mapAccumIndex xs f = do
    mapAccum' (Map.fromAscList . zip [0..] <$> xs) f

mapAccum' :: (DomBuilder t m, PostBuild t m, MonadFix m, MonadHold t m, Ord k, Monoid a) => Dynamic t (Map k v) -> (k -> Dynamic t v -> m (Dynamic t a)) -> m (Dynamic t a)
mapAccum' xs f = do
  r <- listWithKey xs f
  pure (fold <$> joinDynThroughMap r)

dynDyn :: (DomBuilder t m, PostBuild t m, MonadHold t m, Monoid a) => Dynamic t (m (Dynamic t a)) -> m (Dynamic t a)
dynDyn dynWidget = do
    ev <- dyn dynWidget
    dynr <- holdDyn (constDyn mempty) ev
    pure $ join dynr

renderBlocks :: (DomBuilder t m, PostBuild t m, MonadHold t m, MonadFix m, Monoid a) => Config t m a -> Dynamic t [DSum BlockTag Identity] -> ReaderT (Dynamic t Footnotes) m (Dynamic t a)
renderBlocks cfg xs =
  mapAccum xs $ \x -> do
    x' <- factorDyn x
    dynDyn $ fmap (renderBlock cfg) x'

renderBlock :: (DomBuilder t m, PostBuild t m, MonadFix m, MonadHold t m, Monoid a) => Config t m a -> DSum BlockTag (Compose (Dynamic t) Identity) -> ReaderT (Dynamic t Footnotes) m (Dynamic t a)
renderBlock cfg = \case
  ...
  CodeBlockTag :=> Compose dynxs -> let (attr, x) = splitDynPure (runIdentity <$> dynxs) in do
    lift $ _config_renderCode cfg (elCodeHighlighted attr x) attr x
    pure mempty
  RawBlockTag :=> Compose x ->
    lift $ _config_renderRaw cfg (uncurry PandocRawNode_Block . runIdentity <$> x) >> pure mempty
  BlockQuoteTag :=> Compose xs ->
    el "blockquote" $ renderBlocks cfg (fmap runIdentity xs)
  OrderedListTag :=> Compose dynxs -> let (attrs, xss) = splitDynPure (runIdentity <$> dynxs)
  ...
  BulletListTag :=> Compose xss ->
    el "ul" $ mapAccum (runIdentity <$> xss) $ \xs -> el "li" $ renderBlocks cfg xs
  DefinitionListTag :=> Compose defs ->
    el "dl" $
      mapAccum (runIdentity <$> defs) $ \xs -> let (term, descList) = splitDynPure xs in do
        x <- el "dt" $ renderInlines cfg term
        fmap (x <>) $
          mapAccum descList $ \desc ->
            el "dd" $ renderBlocks cfg desc
  HeaderTag :=> Compose dynxs -> let (dynLevel, attr, xs) = splitDyn3Pure (runIdentity <$> dynxs)
                                  in
    dynDyn $ (\level -> elPandocAttr (headerElement level) attr $ do
      renderInlines cfg xs) <$> dynLevel

  HorizontalRuleTag :=> Compose _ ->
    el "hr" blank >> pure mempty
  TableTag :=> Compose dynxs -> let --(attr, _captions, _colSpec, TableHead _ hrows, tbodys, _tfoot) -> do
    ...
    elPandocAttr "table" attr $ do
      x <- el "thead" $ do
        mapAccum hrows $ \dynRows -> let cells = (\(Row _ x) -> x) <$> dynRows in  do
          el "tr" $ do
            mapAccum cells $ \dynCells -> let blks = (\(Cell _ _ _ _ y) -> y) <$> dynCells in do
              el "th" $ renderBlocks cfg (fmap (fmap blockToDSum) blks)
      fmap (x <>) $
        mapAccum tbodys $ \dynBodys -> let rows = (\(TableBody _ _ _ w) -> w) <$> dynBodys in do
          el "tbody" $ do
            mapAccum rows $ \dynRows -> let cells = (\(Row _ z) -> z) <$> dynRows in do
              el "tr" $ do
                mapAccum cells $ \dynCells -> let blks = (\(Cell _ _ _ _ y) -> y) <$> dynCells in do
                  el "td" $ renderBlocks cfg (fmap (fmap blockToDSum) blks)
  DivTag :=> Compose dynxs -> let (attr, xs) = splitDynPure (runIdentity <$> dynxs) in
    elPandocAttr "div" attr $
      renderBlocks cfg xs
  NullTag :=> Compose _ ->
    blank >> pure mempty
  where
    renderInlinesCheckbox (0::Int) (StrTag :=> Compose dynV) = dynDyn $ (\case
...
renderInlines :: (DomBuilder t m, PostBuild t m, MonadFix m, MonadHold t m, Monoid a) => Config t m a -> Dynamic t [DSum InlineTag Identity] -> ReaderT (Dynamic t Footnotes) m (Dynamic t a)
renderInlines cfg xs = do
  mapAccum xs $ \x -> do
    x' <- factorDyn x
    dynDyn $ fmap (renderInline cfg) x'

renderInline :: (DomBuilder t m, PostBuild t m, MonadFix m, MonadHold t m, Monoid a) => Config t m a -> DSum InlineTag (Compose (Dynamic t) Identity) -> ReaderT (Dynamic t Footnotes) m (Dynamic t a)
renderInline cfg = \case
  StrTag :=> Compose x ->
    dynText (runIdentity <$> x) >> pure mempty
  EmphTag :=> Compose xs ->
    el "em" $ renderInlines cfg (runIdentity <$> xs)
  ...
  QuotedTag :=> Compose dynxs -> let (qt, xs) = splitDynPure (runIdentity <$> dynxs) in
    dynText (leftQuote <$> qt) >>renderInlines cfg xs <* dynText (rightQuote <$> qt)
  CiteTag :=> _ -> do
    el "pre" $ text "error[reflex-doc-pandoc]: Pandoc Cite is not handled"
    pure mempty
  CodeTag :=> Compose dynxs -> let (attr, x) = splitDynPure (runIdentity <$> dynxs) in
  ...
  RawInlineTag :=> Compose x ->
    lift $ _config_renderRaw cfg (uncurry PandocRawNode_Inline . runIdentity <$> x) >> pure mempty
  MathTag :=> Compose dynxs ->
    ...
    let (mathType, s) = splitDynPure (runIdentity <$> dynxs)
        ...
        mathDelimRight = \case
          ...
          DisplayMath -> "$$"
    in
      elDynClass "span" (mathTypeCls <$> mathType) $ do
        dynText (mathDelimLeft <$> mathType)
        dynText s
        dynText (mathDelimRight <$> mathType)
        pure mempty
  LinkTag :=> Compose dynxs -> let (attr, xs, l) = splitDyn3Pure (runIdentity <$> dynxs)
                                   (lUrl, lTitle) = splitDynPure l
                                in do
    let attrMap = renderAttr <$> attr
        defaultRender = do
          let attr' = fmap sansEmptyAttrs $ attrMap <> (("href" =:) <$> lUrl) <> (("title" =:) <$> lTitle)
  ...
  NoteTag :=> Compose dynXs -> do
    ...
    dynDyn $ (\xs fs -> case Map.lookup (mkFootnote xs) fs of
      Nothing ->
        ...
        -- aside.
        elClass "aside" "footnote-inline" $ renderBlocks cfg (fmap blockToDSum . runIdentity <$> dynXs)
      Just idx ->
        renderFootnoteRef idx >> pure mempty
          ) <$> (runIdentity <$> dynXs) <*> dynFs
  SpanTag :=> Compose dynxs -> let (attr, xs) = splitDynPure (runIdentity <$> dynxs) in
...
splitDyn3Pure :: Reflex t => Dynamic t (a, b, c) -> (Dynamic t a, Dynamic t b, Dynamic t c)
splitDyn3Pure dynxs = ( (\(x,_,_) -> x) <$> dynxs
                      , (\(_,x,_) -> x) <$> dynxs
                      , (\(_,_,x) -> x) <$> dynxs
                      )
...
blockToDSum = \case
  ...
  OrderedList attrs xss -> OrderedListTag ==> (attrs, fmap (fmap blockToDSum) xss)
  BulletList xss -> BulletListTag ==> fmap (fmap blockToDSum) xss
  DefinitionList defs -> DefinitionListTag ==> fmap (\(x,y) -> (fmap inlineToDSum x, fmap (fmap blockToDSum) y)) defs
  Header level attr xs -> HeaderTag ==> (level, attr, fmap inlineToDSum xs)
  HorizontalRule -> HorizontalRuleTag ==> ()
  Table attr captions colSpec tableHead tbodys tfoot -> TableTag ==> (attr, captions, colSpec, tableHead, tbodys, tfoot)
